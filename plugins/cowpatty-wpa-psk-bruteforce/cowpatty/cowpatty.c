 ...
 *
 * $Id: cowpatty.c,v 1.3 2007-08-20 01:49:19 jspence Exp $
 *
...
int getpacket(struct capture_data *capdata)
{
	...
	if(capdata->pcaptype == DLT_IEEE802_11_RADIO) {
		struct ieee80211_radiotap_header * rt_hdr;

		rt_hdr = (struct ieee80211_radiotap_header *) &packet;
		if(rt_hdr->it_len > h->caplen) {
			/* maybe the capture length is too small? */
			return -1;
		}
		capdata->dot1x_offset = 32 + rt_hdr->it_len;
		capdata->l2type_offset = 30 + rt_hdr->it_len;
		capdata->dstmac_offset = 4 + rt_hdr->it_len;
		capdata->srcmac_offset = 10 + rt_hdr->it_len;
	}
	return (ret);
}
