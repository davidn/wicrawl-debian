 ...
 *
 * $Id: sha1.c,v 1.1 2007-08-04 07:26:10 sith Exp $
 *
			  ...
			  unsigned char *digest, int usecached)
{
	...
	hmac_sha1_vector((unsigned char *)passphrase, passphrase_len, 2, addr,
	    len, tmp, USECACHED);
	memcpy(digest, tmp, SHA1_MAC_LEN);

#ifdef FPGA
	if(usefpga)
		addreg(&cached, digest, passphrase);
	else {
/*
	printf("ipad: ");
	for(i = 0; i < 20; i++)
		printf("%02x ", cached.k_ipad[i]);
	printf("\n");
	printf("opad: ");
	for(i = 0; i < 20; i++)
		printf("%02x ", cached.k_opad[i]);
	printf("\n");
	printf("digest: ");
	for(i = 0; i < 20; i++)
		printf("%02x ", digest[i]);
...
{
	unsigned char zero = 0, counter = 0;
	size_t pos, plen;
	u8 hash[SHA1_MAC_LEN];
	size_t label_len = strlen(label);
	unsigned char *addr[] = { (unsigned char *)label, &zero, data, &counter };
